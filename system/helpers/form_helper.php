...
if ( ! function_exists('form_hidden_by_id'))
{
	function form_hidden_by_id($id, $value = '', $recursing = FALSE)
	{
		static $form;
		...
		if (is_array($id))
		{
			...
			{
				form_hidden_by_id($key, $val, TRUE);
			}
			return $form;
		}
		...
		if ( ! is_array($value))
		{
			$form .= '<input type="hidden" id="'.$id.'" value="'.form_prep($value, $id).'" />'."\n";
		}
		else
		{
			foreach ($value as $k => $v)
