...
{
    if ($apply->sm_status_name == 'approved')
    {
        $url = $this->block->generate_edit_link(site_url('pi/purchase_apply/edit', array($apply->id)));
    }
    else
    {
...
$ops = $this->base_model->fetch_statuses('purchase_apply_status');
$allowed = array('approved', 'approved_and_edited');
$options = array();
$options[' '] = lang('all');
foreach ($ops as $op)
{
    if (in_array($op->status_name, $allowed))
    {
