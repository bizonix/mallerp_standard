...
{
	$i=0;
	foreach ($all_domain as $order)
	{
		$item = array();
		$item[]=$order->domain;
		$order_result=$this->order_model->statistics_order_count($begin_time,$end_time,$order->domain,$statistics_type,$check_user);
		$sale_amount=0;
		$sku_count=0;
		$sku_cost=0;
		$sku_rate=0;
	...
	if ($domain)
	{
		$item = array();
		$item[]=$domain;
		$order_result=$this->order_model->statistics_order_count($begin_time,$end_time,$domain,$statistics_type);
		$sale_amount=0;
		$sku_count=0;
		$sku_cost=0;
		$sku_rate=0;
...
{
	$temp_array[]=round($temp/$all_amount,7)*100;
...
$sortable[] = 'integer';
$sortable[] = 'integer';
echo block_header(lang('sales_statistics'));
echo '<br/>';
...
foreach($all_domain as $order)
{
	$domains[$order->domain]=$order->domain;
}
echo form_dropdown('domain', $domains, isset($domain) ? $domain : '0');
$check_users=array();
$check_users[]=lang('all');
foreach($all_check_user as $user)
{
	$check_users[$user->check_user]=$user->check_user;
}
echo form_dropdown('check_user', $check_users, isset($check_user) ? $check_user : '');
