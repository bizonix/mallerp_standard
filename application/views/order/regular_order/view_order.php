...
$base_url = base_url();
$head = array(
    array('text' => lang('created_date'), 'sort_key' => $sort_key, 'id' => 'order'),
	...
	array('text' => lang('gross'), 'sort_key' => 'gross'),
...
ITEM;
    ...
    for ($i = 0; $i < $count; $i++)
    {
        ...
        {
			$salesrecordnumber='SRN:'.get_salesrecordnumber($order->transaction_id,$item_id,element($i, $skus));
            $link = '<a target="_blank" href="' . $ebay_url . $item_id . '">' . $item_id .'</a>  '.$salesrecordnumber;
        }
        else
        {
            $link = $item_id;
...
$anchor = anchor(
	site_url('shipping/deliver_management/down_order_excel', array($order->id)),
    lang('down_order_excel').'>>'
);
$down_order_excel_html = form_label($anchor);
$down_order_excel_html .= '<br/>';

$anchor = anchor(
	site_url('order/wish_order/complete_sale', array($order->id)),
    'complete sale'
);
$wish_order_complete_sale_html = form_label($anchor);
$wish_order_complete_sale_html .= '<br/>';
...
foreach($order_remarks as $order_remark)
{
	...
	$row[] = $order->auction_site_type."<br>".$order->ebay_id;
	$row[] = $order->currency.":".$gross."<br/>RMB:".$rmb;
	...
	{
		$down_order_excel_html.=$wish_order_complete_sale_html;
	}

    $row[] = $check_html .  $make_pi_html . $close_html . $return_cost_html . $again_html . $auditing_html.$no_consignment_return_cost_html. $move_order_html.$order_remark_html.$down_order_excel_html;
...
$filters = array(
    ...
    array(
		'type'      => 'input',
		//'field'     => 'transaction_id|descript|track_number|sys_remark',
		'field'     => 'transaction_id|track_number',
		'method'    => '=',
	),
    array(
		'type'      => 'input',
