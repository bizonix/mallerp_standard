...
foreach ($scope_statistics as $row)
{
    foreach ($shippers as $shipper)
    {
        ...
        {
            if (isset($statistics[$r]) && isset($statistics[$r][$shipper]))
            {
                $value = $statistics[$r][$shipper];
...
$title = lang('department_package_statistics');
...
{
    $shippers[$shipper->u_name] = $shipper->u_name;
}

$shippers = array_merge(array(lang('all_shippers')), $shippers);

echo form_dropdown('shipper', $shippers, $current_shipper);
$config = array(
    'name'        => 'submit',
    'value'       => lang('submit'),
    'type'        => 'submit',
